...
def food():
    ...
    if((fx,fy) in snakeBody):
        food()
    pygame.draw.rect(screen,food_clour,(fx,fy,width,length))
    score +=1
    if fps < 20:
        fps += 0.2
...
def end(x,y,c=True):
    ...
    if c:
        font = pygame.font.Font('freesansbold.ttf',32)
        text = font.render("YOU CRASHED",True,danger[1],cb)
        tb = text.get_rect()
        tb.center = (resolution[0]//2,resolution[1]//2)
        screen.blit(text,tb)
        pygame.display.update()
        text = font.render(f"Score :{score} ", True, danger[1], cb)
        tb = text.get_rect()
        tb.center = (resolution[0] // 2, resolution[1] //2+50)
        screen.blit(text, tb)
        pygame.display.update()
        text = font.render("press x to exit", True, danger[1], cb)
        tb = text.get_rect()
        tb.center = (resolution[0] // 2, resolution[1] //2-50)
        screen.blit(text, tb)
        pygame.display.update()
...
map_dict = {pygame.K_RIGHT: [True, True], pygame.K_LEFT: [True, False], pygame.K_UP: [False, False],pygame.K_DOWN: [False, True]}
screen = pygame.display.get_surface()
